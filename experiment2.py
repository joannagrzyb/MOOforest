            ...
            for fold_id in range(n_folds):
                for sol_id in range(n_rows_p):
                    if (pareto_solutions[clf_id, fold_id, sol_id, 0] != 0.0) and (pareto_solutions[clf_id, fold_id, sol_id, 1] != 0.0):
                        filename_pareto = "results/experiment2/pareto_raw/%s/%s/fold%d/sol%d.csv" % (dataset_name, clf_name, fold_id, sol_id)
                        if not os.path.exists("results/experiment2/pareto_raw/%s/%s/fold%d/" % (dataset_name, clf_name, fold_id)):
                            os.makedirs("results/experiment2/pareto_raw/%s/%s/fold%d/" % (dataset_name, clf_name, fold_id))
